...
function scrapeInsuranceHeader() {
    ...
    console.log(`[V22] Insurance header — SubID: ${subId}, RP BD: ${rpDob}, Phone: ${carrierPhone}, Responsible: ${responsibleName}`);
    return { subId, rpDob, carrierPhone, responsibleName };
}
...
async function deepCrawlInsurance() {
    ...
    chrome.storage.local.get("audit_context", (result) => {
        ...
        if (headerData.responsibleName !== "N/A" && storedName &&
            !looseNameMatch(headerData.responsibleName, storedName)) {
            identityStale = true;
            console.warn(
                `[V22] Identity mismatch — storage held "${storedName}" but this ` +
        ...
        respParty.dob = headerData.rpDob;
        // If storage was discarded as stale, at least record the name we DID
        // confirm directly from this page's own header, so the file isn't
        // silently blank on identity.
        if (identityStale && !respParty.name) {
            respParty.name = headerData.responsibleName;
        }
        ...
        store.denticon_data = {
            ...
            total_captured:     allPlanAudits.length,
            crawled_at:         new Date().toISOString(),
            identity_mismatch_warning: identityStale
        ...
        chrome.storage.local.set({ audit_context: store }, () => {
            ...
            triggerDownload(store, `Denticon_DeepAudit_${patientName}_${Date.now()}.json`, false);
            const warningSuffix = identityStale
                ? "\n\n⚠ Note: a stale-identity mismatch was detected and auto-corrected — check the console and the 'identity_mismatch_warning' field in the JSON."
                : "";
            alert(`Deep Scrape Complete! Captured ${allPlanAudits.length} plans.${warningSuffix}`);
        });
    });
}
