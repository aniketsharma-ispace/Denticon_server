...
# # D0120 (periodic exam) and D0150 (comprehensive exam) share frequency on
# # this carrier, so a date on either counts for both. This needs to be a
# # real UNION, not a one-directional copy: if D0120 has its own dates AND
# # D0150 has its own dates, both fields should show the combined set, not
# # just whichever code happened to get billed under -- the old code only
# # filled in a code when the OTHER was completely absent, so two codes
# # each already having (different) dates silently never got merged.
#     if "D0120" in claims_history or "D0150" in claims_history:
#         merged = list(dict.fromkeys(
#             claims_history.get("D0120", []) + claims_history.get("D0150", [])
#     ))
#         merged.sort(key=lambda d: datetime.strptime(d, "%m/%d/%Y"), reverse=True)
#         claims_history["D0120"] = list(merged)
#         claims_history["D0150"] = list(merged)
