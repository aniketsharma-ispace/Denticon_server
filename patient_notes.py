...
# def late_dos_dd_ins_history(procedures: list, code: str) -> str:
#     """
#     DD INS site's treatment_history tab has one entry PER procedure code,
#     e.g. {"code": "D0150", "rows": [{"service_date": "04/25/2024", ...}]}.
#     Returns just this code's own on-file date(s); merging in dates from
#     codes listed in "limitation_may_also_apply_to" is handled separately
#     via dynamically-built CODE_ALIASES (see build_patient_notes), since
#     that merge needs to go both directions and apply uniformly through the
#     same _dos_with_aliases mechanism every other carrier uses.
#     """
#     for entry in procedures:
#         if entry.get("code", "").upper() == code.upper():
#             dates = []
#             for row in entry.get("rows", []):
#                 val = (row.get("service_date") or "").strip()
#                 if val and val.lower() != "none apply":
#                     dates.extend([d.strip() for d in val.split(",") if d.strip()])
#             return _format_dates(dates, ("%m/%d/%Y", "%m/%d/%y"))
#     return "NH"
...
#         if i == 0:
#             # First token is always the start of the real carrier name,
#             # even if it's short (e.g. "SUN" in "SUN LIFE") — only the
#             # trailing junk (PPO/CO/PID/etc.) should get cut on length.
#             if w.isalpha():
#                 meaningful.append(w)
#             else:
#                 break
...
#                 break
#         # Ortho Maximum (Used) maps directly to this site's "Lifetime
#         # Individual Maximum" row on the portal's Maximums table — confirmed
#         # against the actual UI, regardless of which treatment types that
#         # lifetime max happens to be tied to for a given plan (Orthodontics,
#         # TMJ, Adjunctive General Services, etc. all surface under the same
#         # "Lifetime Individual Maximum" label on this carrier).
#         ortho_used = None
#         for mx in overview.get("maximums", []):
#             if "lifetime individual maximum" in mx.get("type", "").lower():
#                 ortho_used = fmt_money(mx.get("used", ""))
#                 break
 
#         # No lifetime maximum entry at all on this plan. Don't assume "0.00"
#         # blind — confirm it against plan_provisions instead. This site's
#         # "Orthodontic Age Limit" provision states outright when ortho isn't
#         # covered at all (e.g. "Orthodontic procedures are not a covered
#         # benefit."), which is real portal-confirmed evidence the used
#         # amount is 0, not a guess. Only fall back to "0.00" once that
#         # confirmation is found; if neither a lifetime max entry nor
#         # plan_provisions says anything about ortho, leave it blank rather
#         # than fabricate a number.
#         if ortho_used is None:
#             for prov in dd_ins_data.get("tabs", {}).get("plan_provisions", []):
#                 if "orthodontic" in prov.get("provision_name", "").lower():
#                     desc = prov.get("description", "").lower()
#                     if "not a covered benefit" in desc or "not covered" in desc:
#                         ortho_used = "0.00"
#                     break
...
#     elif is_dd_scraper:
#         tx_procedures = dd_ins_data.get("tabs", {}).get("treatment_history", {}).get("procedures", [])
#         if tx_procedures:
#             procedures = tx_procedures
#             dos_fn = late_dos_dd_ins_history
#         else:
#             # Fallback for scrapes taken before the treatment_history tab
#             # was added — same site, older data shape.
...
#     if is_dd_scraper:
#         # This site's treatment_history rows name their own shared-limitation
#         # codes directly via "limitation_may_also_apply_to" (portal-confirmed,
#         # per-patient/per-plan) — e.g. D0120's row lists D0150 and vice versa,
#         # meaning a date on either code counts for both. Build the alias link
#         # in both directions even when only one side's row happens to mention
#         # it (e.g. D1110 lists D4910 but D4910 has no row of its own to check).
#         # Only wire up codes we actually track/output — no point aliasing
#         # untracked codes like D0145/D0160 that never reach the output fields.
#         tx_procedures = dd_ins_data.get("tabs", {}).get("treatment_history", {}).get("procedures", [])
#         for entry in tx_procedures:
#             src_code = entry.get("code", "").upper()
#             if src_code not in TRACKED_QUERY_CODES:
#                 continue
#             for row in entry.get("rows", []):
#                 shares = row.get("limitation_may_also_apply_to", "")
#                 for other_code in re.findall(r"D\d{4}", shares):
#                     if other_code == src_code or other_code not in TRACKED_QUERY_CODES:
#                         continue
#                     CODE_ALIASES.setdefault(src_code, [])
#                     if other_code not in CODE_ALIASES[src_code]:
#                         CODE_ALIASES[src_code].append(other_code)
#                     CODE_ALIASES.setdefault(other_code, [])
#                     if src_code not in CODE_ALIASES[other_code]:
#                         CODE_ALIASES[other_code].append(src_code)
...
def build_patient_notes(denticon_data: dict, insurance_data: dict) -> dict:
    ...
    if is_metlife:
        ...
        def _is_ppo_network(entry) -> bool:
            return any("delta dental ppo dentist" in n.lower() or "delta dental dpo dentist" in n.lower()for n in entry.get("networks", []))
 
        ind_max_used = "0.00"
        for mx in overview.get("maximums", []):
            if "calendar individual maximum" in mx.get("type", "").lower() and _is_ppo_network(mx):
                ind_max_used = fmt_money(mx.get("used", ""))
                break
 
        ind_ded_used = "0.00"
        for ded in overview.get("deductibles", []):
            if "calendar individual deductible" in ded.get("type", "").lower() and _is_ppo_network(ded):
                ind_ded_used = fmt_money(ded.get("used", ""))
                break
 
        # Ortho Maximum (Used) maps directly to this site's "Lifetime
        # Individual Maximum" row on the portal's Maximums table — confirmed
        # against the actual UI. Only the Delta Dental PPO Dentist network
        # row counts; if this plan has no PPO-network lifetime max row, used
        # is 0.00.
        ortho_used = "0.00"
        for mx in overview.get("maximums", []):
            if "lifetime individual maximum" in mx.get("type", "").lower() and _is_ppo_network(mx):
                ortho_used = fmt_money(mx.get("used", ""))
                break
    ...
    CODE_ALIASES = {
        ...
        "D0274": ["D0272", "D0273", "D0270", "D0277"],  # Bitewings: 4/3/2/1 films, vertical
        "D1206": ["D1208"],
        "D0210": ["D0330"] #h <-> Gel/topical                       # Full mouth series <-> Panoramic
    }
    # MetLife-specific: the plan's own "Cleanings and Periodontal Maintenance"
    # provision states whether Prophylaxis (D1110) and Periodontal Maintenance
    # (D4910) draw from one COMBINED frequency count rather than being tracked
